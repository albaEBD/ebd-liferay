...
<aui:script>
			setInterval(function(){
				...
				var name = "minutes" + '<%=renderResponse.getNamespace()%>';
				name = name.substr(0,name.length-1);
				if(window.localStorage.getItem(name)==null || window.localStorage.getItem(name) != min){
					Liferay.Portlet.refresh('#p_p_id<%=renderResponse.getNamespace()%>');
					window.localStorage.removeItem(name);
					window.localStorage.setItem(name,min);
				}
